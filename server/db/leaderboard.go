...
type PlayerMechSurvives struct {
	...
	MechSurvives int            `db:"count_mech_survives" json:"mech_survives"`
}

func GetPlayerMechSurvives() ([]*PlayerMechSurvives, error) {
	q := `
        WITH bw as (SELECT owner_id, COUNT(mech_id) as count_mech_survives FROM battle_wins bw GROUP BY owner_id ORDER BY COUNT(mech_id) DESC LIMIT 10)
        SELECT p.id, p.username, p.faction_id, p.gid, p.rank, bw.count_mech_survives FROM players p

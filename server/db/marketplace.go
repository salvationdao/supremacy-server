...
var ItemSaleQueryMods = []qm.QueryMod{
	qm.Select(
		...
		fmt.Sprintf(`%s "sold_to.id"`, qm.Rels(soldTooTable, boiler.PlayerColumns.ID)),
		fmt.Sprintf(`%s "sold_to.username"`, qm.Rels(soldTooTable, boiler.PlayerColumns.Username)),
		fmt.Sprintf(`%s "sold_to.faction_id"`, qm.Rels(soldTooTable, boiler.PlayerColumns.FactionID)),
		fmt.Sprintf(`%s "sold_to.public_address"`, qm.Rels(soldTooTable, boiler.PlayerColumns.PublicAddress)),
		fmt.Sprintf(`%s "sold_to.gid"`, qm.Rels(soldTooTable, boiler.PlayerColumns.Gid)),
		...
		//fmt.Sprintf(`%s "mechs.label"`, qm.Rels(boiler.TableNames.Mechs, boiler.MechColumns.la)), //todo fix me
		//fmt.Sprintf(`%s "mech_skin.avatar_url"`, qm.Rels(boiler.TableNames.MechSkin, boiler.MechSkinColumns.)), //todo fix me
		...
		//fmt.Sprintf(`%s AS "weapons.label"`, qm.Rels(boiler.TableNames.Weapons, boiler.WeaponColumns.Label)),
		//fmt.Sprintf(`%s AS "weapons.weapon_type"`, qm.Rels(boiler.TableNames.Weapons, boiler.MysteryCrateColumns.wea)), //todo fix me
		//fmt.Sprintf(`%s AS "weapons.avatar_url"`, qm.Rels(boiler.TableNames.BlueprintWeaponSkin, boiler.MysteryCrateColumns.wea)), //todo fix me
		fmt.Sprintf(`
		(
			CASE
				WHEN %s = 'weapon' THEN COALESCE(wsc.tier, %s)
				WHEN %s = 'mech' THEN COALESCE(msc.tier, %s)
				ELSE %s
			END
	...
	qm.LeftOuterJoin(
		fmt.Sprintf(
			...
			boiler.TableNames.Players,
			soldTooTable,
			qm.Rels(soldTooTable, boiler.PlayerColumns.ID),

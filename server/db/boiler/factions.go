...
var FactionRels = struct {
	...
	BattleQueueBacklogs:     "BattleQueueBacklogs",
...
type factionR struct {
	...
	BattleQueueBacklogs     BattleQueueBacklogSlice     `boiler:"BattleQueueBacklogs" boil:"BattleQueueBacklogs" json:"BattleQueueBacklogs" toml:"BattleQueueBacklogs" yaml:"BattleQueueBacklogs"`
...
// BattleQueueBacklogs retrieves all the battle_queue_backlog's BattleQueueBacklogs with an executor.
func (o *Faction) BattleQueueBacklogs(mods ...qm.QueryMod) battleQueueBacklogQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("\"battle_queue_backlog\".\"faction_id\"=?", o.ID),
	)
...
// LoadBattleQueueBacklogs allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (factionL) LoadBattleQueueBacklogs(e boil.Executor, singular bool, maybeFaction interface{}, mods queries.Applicator) error {
	var slice []*Faction
	var object *Faction
	...
	query := NewQuery(
		...
		qm.WhereIn(`battle_queue_backlog.faction_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		object.R.BattleQueueBacklogs = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.FactionID {
				local.R.BattleQueueBacklogs = append(local.R.BattleQueueBacklogs, foreign)
				if foreign.R == nil {
					foreign.R = &battleQueueBacklogR{}
				}
				foreign.R.Faction = local
...
// AddBattleQueueBacklogs adds the given related objects to the existing relationships
// of the faction, optionally inserting them as new records.
// Appends related to o.R.BattleQueueBacklogs.
// Sets related.R.Faction appropriately.
func (o *Faction) AddBattleQueueBacklogs(exec boil.Executor, insert bool, related ...*BattleQueueBacklog) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.FactionID = o.ID
	...
	if o.R == nil {
		o.R = &factionR{
			BattleQueueBacklogs: related,
		}
	} else {
		o.R.BattleQueueBacklogs = append(o.R.BattleQueueBacklogs, related...)
	}

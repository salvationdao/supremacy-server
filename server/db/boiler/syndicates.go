...
func (o *Syndicate) Templates(mods ...qm.QueryMod) templateQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("\"templates\".\"syndicate_id\"=?", o.ID),
...
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (syndicateL) LoadTemplates(e boil.Executor, singular bool, maybeSyndicate interface{}, mods queries.Applicator) error {
	var slice []*Syndicate
	var object *Syndicate
	...
	query := NewQuery(
		qm.From(`templates`),
		qm.WhereIn(`templates.syndicate_id in ?`, args...),
		qmhelper.WhereIsNull(`templates.deleted_at`),
	)
	if mods != nil {
		mods.Apply(query)
...
// Sets related.R.Syndicate appropriately.
func (o *Syndicate) AddTemplates(exec boil.Executor, insert bool, related ...*Template) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.SyndicateID = o.ID

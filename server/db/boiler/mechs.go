...
func (o *Mech) BattleQueues(mods ...qm.QueryMod) battleQueueQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("\"battle_queue\".\"mech_id\"=?", o.ID),
...
func (o *Mech) TelegramNotifications(mods ...qm.QueryMod) telegramNotificationQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("\"telegram_notifications\".\"mech_id\"=?", o.ID),
	)
...
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (mechL) LoadBattleQueues(e boil.Executor, singular bool, maybeMech interface{}, mods queries.Applicator) error {
	var slice []*Mech
	var object *Mech
...
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (mechL) LoadTelegramNotifications(e boil.Executor, singular bool, maybeMech interface{}, mods queries.Applicator) error {
	var slice []*Mech
	var object *Mech
	...
	query := NewQuery(
		...
		qm.WhereIn(`telegram_notifications.mech_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
// Sets related.R.Mech appropriately.
func (o *Mech) AddBattleQueues(exec boil.Executor, insert bool, related ...*BattleQueue) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.MechID = o.ID
...
// Sets related.R.Mech appropriately.
func (o *Mech) AddTelegramNotifications(exec boil.Executor, insert bool, related ...*TelegramNotification) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.MechID = o.ID

...
# run migrations
ssh root@sale.supremacy.fi 'cd /home/gameserver/ && source ./gameserver-staging.env && ./migrate -database "postgres://$GAMESERVER_DATABASE_USER:$GAMESERVER_DATABASE_PASS@$GAMESERVER_DATABASE_HOST:$GAMESERVER_DATABASE_PORT/$GAMESERVER_DATABASE_NAME?sslmode=disable" -path ./migrations drop -f'
# run migrations
ssh root@sale.supremacy.fi 'cd /home/gameserver/ && source ./gameserver-staging.env && ./migrate -database "postgres://$GAMESERVER_DATABASE_USER:$GAMESERVER_DATABASE_PASS@$GAMESERVER_DATABASE_HOST:$GAMESERVER_DATABASE_PORT/$GAMESERVER_DATABASE_NAME?sslmode=disable" -path ./migrations up'
# run seed
ssh root@sale.supremacy.fi 'cd /home/gameserver/ && ./gameserver db'
